    ...
    return (
        <div className="animate-fade-in">
            <header className="text-center mb-8 sm:mb-12">
                <h1 className="text-3xl sm:text-4xl md:text-5xl font-bold text-gray-100">مساعد محطة المياه الذكي</h1>
                <p className="text-base sm:text-lg text-gray-400 mt-4">اختر الأداة التي تناسب مهمتك اليوم.</p>
            </header>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 md:gap-8">
                {sections.map(section => (
                    <button
                        ...
                        className={`bg-gray-800/50 border border-gray-700 rounded-xl p-4 sm:p-6 text-center transition-all duration-300 transform 
                    ...
                    >
                        <div className="flex justify-center items-center text-cyan-400 mb-4">
                            {React.cloneElement(section.icon, { className: "h-12 w-12 sm:h-16 sm:h-16" })}
                        </div>
                        <h2 className="text-lg sm:text-xl font-bold text-gray-100 mb-2">{section.title}</h2>
