...
const ChatInterface: React.FC<ChatInterfaceProps> = ({ onBack, title, subtitle, storageKey, analyzeQueryFn, liveSessionSystemInstruction, thinkingMessage }) => {
    ...
    return (
        <div className="flex h-full lg:gap-4 relative overflow-hidden">
             <aside className={`
                ...
                <div className="flex-grow bg-gray-900 rounded-lg p-4 overflow-y-auto mb-4 min-h-[200px]">
                    ...
                    {chatHistory.map((msg) => (
                        <div key={msg.id} className={`flex my-3 items-end gap-2 ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                            <div className={`max-w-[85%] sm:max-w-[80%] p-3 rounded-xl shadow ${msg.role === 'user' ? 'bg-cyan-500 text-white rounded-br-none' : 'bg-gray-700 text-gray-200 rounded-bl-none'}`}>
                ...
                <div className="flex-shrink-0">
                     {imagePreview && (
                        <div className="relative self-start mb-2 ml-16 transform transition-all duration-300 ease-out animate-fade-in-up-sm">
                            <img src={imagePreview.url} alt="معاينة الصورة" className="h-16 w-16 sm:h-20 sm:w-20 rounded-lg object-cover border-2 border-gray-600 shadow-md" />
                             {isUploading && (
                    ...
                    <form onSubmit={handleSendMessage} className="flex items-end gap-1 sm:gap-2 p-2 bg-gray-700 rounded-lg">
                        <input
                            type="file"
                            ref={fileInputRef}
                        ...
                        <button 
                            ...
                            className="p-2.5 rounded-lg bg-gray-600 text-gray-200 hover:bg-gray-500 transition disabled:opacity-50 flex-shrink-0"
                            disabled={isThinking || isSessionActive || isUploading}
                            title="رفع صورة"
                        >
                            <CameraIcon />
                        ...
                        />
                         <button 
                            type="submit"
                            className="p-2.5 rounded-lg bg-cyan-500 text-white hover:bg-cyan-600 transition disabled:opacity-50 flex-shrink-0"
                        ...
                        >
                         ...
                         <button 
                            ...
                            className={`p-2.5 rounded-lg transition flex-shrink-0 text-white ${isSessionActive ? 'bg-red-600 hover:bg-red-700 animate-pulse' : 'bg-gray-600 hover:bg-gray-500'} disabled:opacity-50`}
            ...
            {conversationToDelete && (
                <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                    <div className="bg-gray-800 border border-red-500/20 rounded-lg shadow-2xl p-4 sm:p-6 max-w-sm w-full text-center transform transition-all animate-fade-in-up">
                        <h3 className="text-base sm:text-lg font-bold text-gray-100 mb-2">تأكيد الحذف</h3>
